...
# 3. AI LOGIC: EXTRACTION
def get_clean_question(llm, raw_text):
    template = """
    Extract the Security+ question EXACTLY. If not a question, return: SKIP.
    TEXT: {raw_text}
    FORMAT:
    QUESTION: [text]
    A: [text]
...
def get_tutor_feedback(llm, question, user_ans, correct_letter, is_right):
    ...
    Explain why the correct answer is the BEST choice for the SY0-701 exam.
    If the student was wrong, explain the flaw in their choice.
    """
    prompt = PromptTemplate.from_template(template)
    chain = prompt | llm | StrOutputParser()
